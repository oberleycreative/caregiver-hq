...
function sf_register_settings() {
	...
	add_settings_field('sf_sidebar_pos', "Sidebar position", 'sf_sidebar_pos_str', 'sf', 'sf_appearance');
	...
	add_settings_field('sf_blur_content', "Blur main content when sidebar is exposed", 'sf_blur_content_str', 'sf', 'sf_appearance');
	add_settings_field('sf_side_stroke', "Visible sidebar edge when it's hidden", 'sf_side_stroke_str', 'sf', 'sf_appearance');
	...
	add_settings_field('sf_tab_logo', "Top image (logo)", 'sf_tab_logo_str', 'sf', 'sf_appearance', array('chapter' => 'Sidebar additional elements'));
...
//	add_settings_field('sf_lh', "Line-height of menu item:", 'sf_lh_str', 'sf', 'sf_menu_items');
	...
	add_settings_field('sf_separators_width', "Separators width relative to sidebar width in percentage:", 'sf_separators_width_str', 'sf', 'sf_menu_items');
	...
	add_settings_field('sf_underline', "Item underline on hover:", 'sf_underline_str', 'sf', 'sf_menu_items');
	add_settings_field('sf_underline_color', "Underline color:", 'sf_underline_color_str', 'sf', 'sf_menu_items');
	...
	add_settings_field('sf_mob_nav', "Navbar for mobiles with screen smaller than 782px:", 'sf_mob_nav_str', 'sf', 'sf_label');
	...
	add_settings_field('sf_label_color', "Button color", 'sf_label_color_str', 'sf', 'sf_label', array('chapter' => 'Stylings'));
...
function sf_display_str() {
	...
	<div class='loc_popup'>
		...
		<div style="height:180px; overflow:auto; border:1px solid #dfdfdf; padding:5px; box-sizing:border-box;margin-bottom:5px;">
...
function sf_tab_logo_str() {
	$options = sf_get_options();
	echo '<h6>Image will be shrinked to 70px height, use images with minimum 2x height for retina displays and mobiles.</h6>';
	if (empty($options['sf_tab_logo'])) {
...
function sf_sidebar_style_str() {
    ...
    <p style='margin-top:5px' data-notice='full'> Bonus! Stylish fullscreen overlay menu. It will show only first-level menu items</p>
    ";
...
function sf_side_stroke_str() {
	...
	echo "
		<h6>Thin vertical line on screen edge.</h6>
...
function sf_font_str() {
	...
	echo "<h6>Font settings for primary text in menu items. Tip: choose text color for each level on Appearance tab</h6><select id='sf_font' name='sf_options[sf_font]'>
...
function sf_uppercase_str() {
	...
	$first_checked = $style === 'yes' ? 'checked' : '';
	$sec_checked = $style === 'no' ? 'checked' : '';

	echo "
    <span id='sf_uppercase_chooser'  class='chooser'>
    	<input id='sf_uppercase_yes' name='sf_options[sf_uppercase]'  type='radio' value='yes' {$first_checked} style='' /><label for='sf_uppercase_yes'>Aa</label><input id='sf_uppercase_no' name='sf_options[sf_uppercase]'  type='radio' value='no' {$sec_checked} style='' /><label for='sf_uppercase_no'>AA</label>
    </span>";
}
...
function sf_blur_content_str () {
    ...
    $sec_checked = $options['sf_blur_content'] == 'none' ? 'checked="checked"' : '';
	  ...
	  echo "<h6>If browser supports this CSS filter.</h6>";
...
function sf_sidebar_pos_str () {
	...
	echo "<h6>For vertical sidebar mode.</h6>";
